// FIR Filter Verilator+Renode Integration (AXI-Lite target)
...
Vfir_filter_axil_cosim *top = new Vfir_filter_axil_cosim;
VerilatedVcdC *tfp;
...
RenodeAgent *initAgent() {
    ...
    agent->addBus(bus);
    if(cosim_verbose) {
        agent->log(LOG_LEVEL_INFO, "fir_cosim: AXI-Lite wired (DATA=64, ADDR=32)");
    }
    return agent;
}
...
RenodeAgent *Init() {
    ...
    if(cosim_verbose) {
        host_print("[fir_cosim] Init() called (pid=%d)", getpid());
    }
    RenodeAgent *agent = initAgent();
    if(cosim_verbose) agent->log(LOG_LEVEL_INFO, "fir_cosim: connecting native");
    agent->connectNative();
    if(cosim_verbose) agent->log(LOG_LEVEL_INFO, "fir_cosim: Native connection established");
    return agent;
}
